<?php $this->load->view('partials/_breadcrumb', [
	'breadcrumbs' => [
		...
		'create' => 'master/payment-type/edit/' . $paymentType['id']
	]
]) ?>

<h4 class="text-primary"><?= $title ?></h4>

<form action="<?= site_url('master/payment-type/update/' . $paymentType['id']) ?>" method="POST" id="form-payment-type" class="edit">
    <?= _csrf() ?>
    <?= _method('put') ?>
	...
	<div class="form-group">
		...
		<input type="text" class="form-control" id="payment_type" name="payment_type" required maxlength="50"
			   value="<?= set_value('payment-type', $paymentType['payment_type']) ?>" placeholder="Payment title">
		<?= form_error('payment-type') ?>
	</div>
	<div class="form-group">
		<label for="description">Description</label>
		<textarea class="form-control" id="description" name="description" maxlength="500"
				  placeholder="Payment type description"><?= set_value('description', $paymentType['description']) ?></textarea>
			...
			<tr>
				...
				<td>
					<div class="input-group">
						<input type="number" step="1" min="0" max="100" value="<?= set_value('services[' . $service['id'] . '][payment_percent]', numerical($service['payment_percent'])) ?>"
				...
				<td>
					<div class="input-group">
						<input type="number" step="any" min="0" max="100" value="<?= set_value('services[' . $service['id'] . '][margin_percent]', $service['margin_percent']) ?>" class="form-control" placeholder="Margin percent"
